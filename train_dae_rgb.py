...
def train(dataset, learn_step=0.005,
    ...
    if dae_dict['kind'] == 'standard':
        dae = buildDAE(input_concat_h_vars, input_mask_var, nb_in_channels,
                       trainable=True, void_labels=void_labels,
                       load_weights=resume, path_weights=loadpath,
                       model_name='dae_model.npz', out_nonlin=linear,
                       concat_h=dae_dict['concat_h'], noise=dae_dict['noise'],
                       n_filters=dae_dict['n_filters'],
        ...
        dae = buildFCN8_DAE(input_concat_h_vars, input_mask_var, nb_in_channels,
                            nb_in_channels=nb_in_channels, path_weights=loadpath,
        ...
        dae = buildDAE_contextmod(input_concat_h_vars, input_mask_var, nb_in_channels,
                                  path_weights=loadpath,
                                  model_name='dae_model.npz',
                                  trainable=True, load_weights=resume,
                                  noise=dae_dict['noise'],
                                  concat_h=dae_dict['concat_h'], out_nonlin=linear)
    else:
